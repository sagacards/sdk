// Basic random card draw methods.
...
export default async function drawCard(principal: string): Promise<CardDraw> {
    ...
    const upsidedown =
        F_UPSIDEDOWN === undefined ? Math.random() >= 0.66 : F_UPSIDEDOWN;
    const message = `Drew ${cardName} ${
        upsidedown ? 'up-side-down' : 'right-side-up'
    } (${card})`;
...
function allowAdminForcedDraw(): [number | undefined, boolean | undefined] {
    ...
    const [F_CARD, F_UPSIDEDOWN] = global?.FORCE_NEXT_DRAW || [
        undefined,
        undefined,
    ];
